...
ChipsAndGuac.prototype.getOrders = function() {
  ...
  return self.loginIfNeeded()
    ...
    .spread(bodyOrError)
    .then(function(html) {
      console.log('location ' + self._locationId + order.location + ' homepage success!');
...
var parsed_pickup = Date.parse(orderDetails.pickupTimes[0]);
var nowtime_parsed = Date.parse(nowtime); 
var waittime_min = Math.round(((Math.abs(parsed_pickup - nowtime_parsed))/60000)-240);
...
if(waittime_min > 60) {
console.log("Your pickup time would be " + waittime_min + " minutes");
console.log("THAT IS AN UNACCEPTABLY LONG TIME TO WAIT FOR A BURRITO!! BETTER NO CHIPOTLE THAN SLOW CHIPOTLE!SHUTTING DOWN NOW");
return orderDetails;
}
...
var nowtime_parsed = Date.parse(nowtime); 
var waittime_min = Math.round(((Math.abs(parsed_pickup - nowtime_parsed))/60000)-240);
console.log("Your pickup time is in " + waittime_min + " minutes");
            return;
          } else {
    ...
    .then(function(token) {
      ...
      .then(function(body) { 
        if(body.IsSuccessful) {
          ...
          console.log('Items from favorited meal %s added to order %s', orderId, self._currentOrderId);
...
ChipsAndGuac.prototype.selectPayment = function() {
  ...
  return self.request.getAsync({uri: BASE_ORDER_URL + '/Payment/Index/512/' + self._currentOrderId})
    ...
    .then(function(body) {
      if(body.IsSuccessful) {
        console.log('Pay in store option selected');
        return true;
      } else {
...
ChipsAndGuac.prototype.login = function() {
  ...
  console.log("Starting CHIPOTLE AUTOBURRITO BOT...");
  console.log("Logging in to your Chipotle Account at "  + self._email + "...");
  return self.request.getAsync({uri: BASE_ORDER_URL})
    .get(1)
    .then(getActionToken)
    .then(function(token) {
    ...
    .then(function(body){
      if(body.IsSuccessful) {
        console.log("Login success! Connecting to local Chipotle Restaurant #" + self._locationId + "...");
        return;
      } else {
